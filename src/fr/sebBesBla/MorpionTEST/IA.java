package fr.sebBesBla.MorpionTEST;

public class IA {
	int taillePlateau;
	double[][] plateauBase;		// La valeur de chaque case est fonction de son potentiel
							// ŕ faire une ligne et de sa proximité du centre.
	double[][] plateau;
	...
	public IA(int taille) {
		...
		// Donne ŕ chaque case une valeur de 2.
	...
	public void analyser(Plateau p) {
		...
		for (row=0; row<taillePlateau; row++) {
			...
			if (nbX == taillePlateau - 1) {					// Empęche l'adversaire de terminer une ligne.
				for (col=0; col<taillePlateau; col++) {
					plateau[row][col] *= 100.0;
				}
			} else if (nbO == 0 && nbX > 0) {				// Bloque l'adversaire de maničre anticipée.
				for (col=0; col<taillePlateau; col++) {
					plateau[row][col] *= nbX;
				}
